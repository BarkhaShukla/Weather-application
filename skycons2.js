...
function weatherReport(latitude, longitude) {
	...
	var apiKey       = '6d34c5bc35377efc0e256a3dac4f849c',
	...
	$.getJSON(api_call, function(forecast) {
		...
		for(var i = 0, l = forecast.daily.data.length; i < l - 1; i++) {
			...
			humidityA.push(forecast.daily.data[i].humidity);
			tempA.push(Math.round(forecast.hourly.data[i].temperature));
			speedA.push(forecast.daily.data[i].windSpeed);
		...
		$("#forecast").append('<h1>WEATHER HISTORY CHARTS : </h1>')
		$("#forecast").append('<div id="containerH"></div>')
		$("#forecast").append('<div id="containerT"></div>')
		$("#forecast").append('<div id="containerW"></div>')

		Highcharts.chart('containerH', {

		    title: {
		        text: 'Humidity history for this area:'
		    },
		...
		Highcharts.chart('containerW', {
		    ...
		    title: {
		        text: 'Wind history for this area:'
		    },
		...
		Highcharts.chart('containerT', {
		    ...
		    title: {
		        text: 'Temperature history for this area:'
		    },
...
$('#lblRecent').on('click', function (event) {
...
$('button').on('click', function (e) {
	...
	sessionStorage.city_name = city_name;

	
	
	fVal = sessionStorage.city_name + '|' + sessionStorage.long + '|' + sessionStorage.lat;


	var value = fVal;
	$("#lblRecent").text(value === undefined || value === null ? "" : value);
...
function insertGoogleScript() {
	...
	api_key    = 'AIzaSyAEE5ZchNxQd9lIwlJz3IqhST0s_n-wOZs';
